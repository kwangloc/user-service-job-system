...
const { City, validateCity} = require("./api/v1/models/cityModel.js");

async function insertProvincesWithIndexes() {
    try {
        const cities = [
        ...
        ].map((city, index) => ({ index: index + 1, city }));
        const result = await City.insertMany(cities);
        console.log(`${result.insertedCount} cities with indexes were successfully inserted.`);
    } catch (err) {
        console.error('Error inserting provinces:', err);
    } 
}
insertProvincesWithIndexes();
// console.log(cities);